...
int SCRFD::loadModel(AAssetManager *assetManager)
{
  return 0;
}
#else
int SCRFD::loadModel(const char *model_bin, const char *model_param)
{
  scrfd.clear();
  ...
  scrfd.opt = option_;
  scrfd.load_param(model_param);
  scrfd.load_model(model_bin);

  return 0;
}
#endif

int SCRFD::detect(cv::Mat &src, std::vector<FaceBox> &boxes, float prob_threshold, float nms_threshold)
{
  ...
  ncnn::Mat in = ncnn::Mat::from_pixels_resize(src.data, ncnn::Mat::PIXEL_BGR2RGB, width, height, w, h);
  ...
  boxes.resize(face_count);
  for (int i = 0; i < face_count; i++)
  {
    FaceObject &face_obj = faceproposals[picked[i]];

    // adjust offset to original unpadded
    float x0 = (face_obj.rect.x - (wpad / 2)) / scale;
    float y0 = (face_obj.rect.y - (hpad / 2)) / scale;
    float x1 = (face_obj.rect.x + face_obj.rect.width - (wpad / 2)) / scale;
    float y1 = (face_obj.rect.y + face_obj.rect.height - (hpad / 2)) / scale;
    ...
    cv::rectangle(src, face_obj.rect, cv::Scalar(0, 255, 0), 3);
    FaceBox box;
    box.confidence = face_obj.prob;
    box.x1 = x0;
    box.y1 = y0;
    box.x2 = x1;
    box.y2 = y1;
    ...
    {
      float x0 = (face_obj.landmark[0].x - (wpad / 2)) / scale;
      float y0 = (face_obj.landmark[0].y - (hpad / 2)) / scale;
      float x1 = (face_obj.landmark[1].x - (wpad / 2)) / scale;
      float y1 = (face_obj.landmark[1].y - (hpad / 2)) / scale;
      float x2 = (face_obj.landmark[2].x - (wpad / 2)) / scale;
      float y2 = (face_obj.landmark[2].y - (hpad / 2)) / scale;
      float x3 = (face_obj.landmark[3].x - (wpad / 2)) / scale;
      float y3 = (face_obj.landmark[3].y - (hpad / 2)) / scale;
      float x4 = (face_obj.landmark[4].x - (wpad / 2)) / scale;
      float y4 = (face_obj.landmark[4].y - (hpad / 2)) / scale;

      face_obj.landmark[0].x = std::max(std::min(x0, (float)width - 1), 0.f);
      face_obj.landmark[0].y = std::max(std::min(y0, (float)height - 1), 0.f);
      face_obj.landmark[1].x = std::max(std::min(x1, (float)width - 1), 0.f);
      face_obj.landmark[1].y = std::max(std::min(y1, (float)height - 1), 0.f);
      face_obj.landmark[2].x = std::max(std::min(x2, (float)width - 1), 0.f);
      face_obj.landmark[2].y = std::max(std::min(y2, (float)height - 1), 0.f);
      face_obj.landmark[3].x = std::max(std::min(x3, (float)width - 1), 0.f);
      face_obj.landmark[3].y = std::max(std::min(y3, (float)height - 1), 0.f);
      face_obj.landmark[4].x = std::max(std::min(x4, (float)width - 1), 0.f);
      face_obj.landmark[4].y = std::max(std::min(y4, (float)height - 1), 0.f);

      for (int i = 0; i < 5; i++)
        cv::circle(src, cv::Point(face_obj.landmark[i].x, face_obj.landmark[i].y), 2, cv::Scalar(255, 0, 0), 2);
    }
  }
